...
function AnimatedCounter({ target, suffix = '%', color, duration = 1800 }: { target: number; suffix?: string; color: string; duration?: number }) {
    ...
    return (
        <p
            ref={ref}
            className="font-['Inter'] font-bold leading-[normal] not-italic text-[56px] transition-colors duration-300"
            style={{ color }}
        >
            {count}{suffix}
        </p>
...
export function ImpactCard({ value, target, suffix = "", description, themeColor, maxWidth, reverseLayout = false }: ImpactCardProps) {
    ...
    const ValueComponent = typeof finalValue === 'number'
        ...
        : <span className="font-['Inter'] font-bold text-[56px] leading-[normal] transition-colors duration-300" style={{ color: isHovered ? '#ffffff' : '#000000' }}>{finalValue}{finalSuffix}</span>;

    const DescriptionComponent = (
        <p
            className="font-['Inter'] font-medium leading-[normal] not-italic text-[20px] whitespace-pre-wrap transition-colors duration-300"
            style={{
